...
- (void)layoutSubviews
{
    ...
    if (!self.isResizing) {
        ...
        if (self.interfaceOrientation != interfaceOrientation) {
            [self zoomToCropRect:self.scrollView.frame andCenter:NO manually:layouted ? YES : NO];
            if (!layouted)
                layouted = YES;
        }
    }
...
- (void)setCropAspectRatio:(CGFloat)aspectRatio andCenter:(BOOL)center
{
    ...
    [self zoomToCropRect:cropRect andCenter:center manually:NO];
}
...
- (void)zoomToCropRect:(CGRect)toRect andCenter:(BOOL)center manually:(BOOL)manually
{
    if (CGRectEqualToRect(self.scrollView.frame, toRect) && !manually) {
        return;
    }
...
{
    [self zoomToCropRect:toRect andCenter:NO manually:NO];
}
