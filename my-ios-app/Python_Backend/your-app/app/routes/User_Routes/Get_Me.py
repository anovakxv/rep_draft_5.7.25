...
def get_user_response(user, session_user_id=None):
    ...
    # Add skills — select only titles to avoid loading full Skill objects
    skill_titles = db.session.query(Skill.title).join(
        UserSkill, Skill.id == UserSkill.skills_id
    ).filter(UserSkill.users_id == user.id).all()
    user_row['skills'] = [title for (title,) in skill_titles]
