...
class ProfileViewModel: ObservableObject {
    ...
    @Published var previousWrite: String = ""
...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                StatusBarView()
                NavigationHeaderView(name: viewModel.user.fullName, onBack: viewModel.goBack)
                ProfileInfoView(
                ...
                Group {
                    if selectedTab == 0 {
                        ...
                        WriteContentView(
                            previousWrite: viewModel.previousWrite,
...
struct WriteContentView: View {
    let previousWrite: String
    @Binding var writeText: String
    let isCurrentUser: Bool
    let onSave: () -> Void

    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            if !previousWrite.isEmpty {
                Text(previousWrite)
                    .font(.body)
                    .padding()
